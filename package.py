...
def connect_mongodb():
    """Conecta ao MongoDB usando pymongo com configurações otimizadas"""
    URI = st.secrets["uri"]
    ...
    try:
        ...
        st.success("✅ Conexão MongoDB estabelecida com sucesso")
        return client
...
def main():
    ...
    with tabs[0]:
        with st.form("form_novo_gasto", clear_on_submit=True):
            ...
            pagamento_compartilhado = st.selectbox("Tipo de Despesa", [
                "Compra individual", "Nossa", "Cada uma pagou metade"
            ])
        ...
        if submitted:
            try:
                ...
                if pagamento_compartilhado == "Cada uma pagou metade":
                    valor_total = round(valor_total_original / 2, 2)
                else:
                ...
                def gerar_pendencia(buyer, pagamento_compartilhado):
                    ...
                    if pagamento_compartilhado == "Nossa":
                        ...
                        return {"tem_pendencia": True, "devedor": devedor, "valor_pendente": round(valor_total / 2, 2), "status_pendencia": "em aberto"}
                    return {"tem_pendencia": False, "devedor": None, "valor_pendente": None, "status_pendencia": None}
                ...
                documento = {
                    ...
                    "attachment": anexo, "installment": int(total_parcelas), "createdAt": datetime.now(),
                    "pagamento_compartilhado": pagamento_compartilhado,
                    "tem_pendencia": pendencia_info["tem_pendencia"], "devedor": pendencia_info["devedor"],
                    "valor_pendente": pendencia_info["valor_pendente"], "status_pendencia": pendencia_info["status_pendencia"]
                }
                ...
                if pagamento_compartilhado == "Cada uma pagou metade":
                    ...
                    doc_outra.pop("_id", None)
                    doc_outra["buyer"] = outra_pessoa
                    doc_outra["registrado_por"] = compradora
    ...
    with tabs[1]:
        ...
        with st.form("form_filtro_datas_pietrah", clear_on_submit=False):
            ...
            with col1:
                data_inicio_tmp = st.date_input("Data inicial", value=st.session_state["p_data_inicio"],
                    min_value=data_min_df, max_value=data_max_df, format="DD/MM/YYYY", key="p_data_inicio_input")
            with col2:
                data_fim_tmp = st.date_input("Data final", value=st.session_state["p_data_fim"],
                    min_value=data_min_df, max_value=data_max_df, format="DD/MM/YYYY", key="p_data_fim_input")
        ...
        if aplicar:
            ...
            st.session_state["p_modo_periodo"] = "mes_atual" if data_inicio_tmp == default_ini and data_fim_tmp == default_fim else "custom"
            st.session_state["p_filtro_aplicado"] = True
        ...
        em_aberto = df[(df["devedor"] == "Pietrah") & (df["status_pendencia"] == "em aberto")]["valor_pendente"].sum()
        a_receber = df[(df["devedor"] == "Susanna") & (df["status_pendencia"] == "em aberto")]["valor_pendente"].sum()
        ...
        gastos_por_categoria = df.loc[mask_pietrah].groupby("label")["total_value"].sum().reset_index().sort_values(by="total_value", ascending=False)
        ...
        if total_geral == 0:
            ...
            fig = px.pie(gastos_por_categoria, names="label", values="total_value", hole=0.5)
            fig.update_traces(textinfo="label+value+percent", texttemplate="%{label}<br>R$ %{value:,.2f}<br>%{percent}",
                textposition="outside", pull=[0.03]*len(gastos_por_categoria), marker=dict(line=dict(color="#000000", width=0.5)))
            fig.update_layout(title="", title_x=0.5, showlegend=False, margin=dict(t=40, b=80, l=40, r=40))
        ...
        categorias_selecionadas = st.multiselect("Filtrar por categoria", options=sorted(categorias_disponiveis), key="p_multiselect_categorias")

        df_filtrado = df[mask_pietrah & (df["label"].isin(categorias_selecionadas))][["createdAt", "label", "item", "description", "total_value"]]
        df_filtrado = df_filtrado.sort_values(by="createdAt", ascending=False)
        total_visivel = df_filtrado["total_value"].sum()
        linha_total = pd.DataFrame([{"createdAt": None, "label": "TOTAL", "item": "", "description": "", "total_value": total_visivel}])
        ...
        a_receber_df = df[(df["devedor"] == "Susanna") & (df["status_pendencia"] == "em aberto")]
        total_receber = a_receber_df["valor_pendente"].sum()

        if not a_receber_df.empty:
            df_mostrar = a_receber_df.rename(columns={"item": "Item", "label": "Categoria", "description": "Descrição", "valor_pendente": "Valor Pendente"})[["Item", "Categoria", "Descrição", "Valor Pendente"]]
    ...
    with tabs[2]:
        ...
        if not df.empty:
            ...
            with st.form("form_filtro_datas", clear_on_submit=False):
                col1, col2 = st.columns(2)
                with col1:
                    data_inicio_tmp = st.date_input("Data inicial", value=st.session_state["data_inicio"],
                        min_value=data_min_df, max_value=data_max_df, format="DD/MM/YYYY")
                with col2:
                    data_fim_tmp = st.date_input("Data final", value=st.session_state["data_fim"],
                        min_value=data_min_df, max_value=data_max_df, format="DD/MM/YYYY")
                aplicar = st.form_submit_button("Aplicar filtro")
            ...
            if aplicar:
                ...
                st.session_state["modo_periodo"] = "mes_atual" if data_inicio_tmp == default_ini and data_fim_tmp == default_fim else "custom"
                st.session_state["filtro_aplicado"] = True
            ...
            em_aberto = df[(df["devedor"] == "Susanna") & (df["status_pendencia"] == "em aberto")]["valor_pendente"].sum()
            a_receber = df[(df["devedor"] == "Pietrah") & (df["status_pendencia"] == "em aberto")]["valor_pendente"].sum()
            ...
            gastos_por_categoria = df[df["buyer"] == "Susanna"].groupby("label")["total_value"].sum().reset_index().sort_values(by="total_value", ascending=False)
            total_geral = gastos_por_categoria["total_value"].sum()
            gastos_por_categoria["percentual"] = gastos_por_categoria["total_value"] / total_geral
            gastos_por_categoria["percent_texto"] = gastos_por_categoria["percentual"].apply(lambda x: f"{x:.0%}")
            gastos_por_categoria["pct"] = gastos_por_categoria["total_value"] / gastos_por_categoria["total_value"].sum()

            def fmt_brl(v):
                return f"R$ {v:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")

            gastos_por_categoria["valor_fmt"] = gastos_por_categoria["total_value"].apply(fmt_brl)
            gastos_por_categoria["legenda"] = gastos_por_categoria.apply(lambda r: f"{r['label']} — {r['valor_fmt']}", axis=1)

            LIMIAR_FORA = 0.06
            text_positions = ["outside" if p < LIMIAR_FORA else "inside" for p in gastos_por_categoria["pct"]]
            text_templates = ["%{label}<br>%{percent}" if pos == "outside" else "%{percent}" for pos in text_positions]
            pull_vals = [0.04 if p < LIMIAR_FORA else 0 for p in gastos_por_categoria["pct"]]

            fig = px.pie(gastos_por_categoria, names="legenda", values="total_value", hole=0.5)
            fig.update_traces(textinfo="none", textposition=text_positions, texttemplate=text_templates,
                insidetextorientation="radial", pull=pull_vals, marker=dict(line=dict(color="white", width=1)))
            fig.update_layout(title="Distribuição de gastos por categoria", title_x=0.5, showlegend=True,
                legend_title_text="", legend=dict(orientation="v", yanchor="middle", y=0.5, xanchor="left", x=1.02, font=dict(size=12)),
                uniformtext_minsize=11, uniformtext_mode="hide", margin=dict(t=60, b=40, l=40, r=180), font=dict(size=13))
            st.plotly_chart(fig, use_container_width=True)
            st.divider()

            st.subheader("🧾 Detalhamento dos gastos de Susanna")
            categorias_disponiveis = df[df["buyer"] == "Susanna"]["label"].dropna().unique().tolist()
            categorias_selecionadas = st.multiselect("Filtrar por categoria", options=sorted(categorias_disponiveis), default=sorted(categorias_disponiveis))

            df_filtrado = df[(df["buyer"] == "Susanna") & (df["label"].isin(categorias_selecionadas))][["createdAt", "label", "item", "description", "total_value"]]
            df_filtrado = df_filtrado.sort_values(by="createdAt", ascending=False)
            total_visivel = df_filtrado["total_value"].sum()
            linha_total = pd.DataFrame([{"createdAt": None, "label": "TOTAL", "item": "", "description": "", "total_value": total_visivel}])
            df_exibicao = pd.concat([df_filtrado, linha_total], ignore_index=True)
